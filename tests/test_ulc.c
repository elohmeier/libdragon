/**
 * Source-free regression for ULC mono output saturation.
 *
 * Sends synthetic Q1.14 coefficients/lap state to the installed ULC RSP
 * overlay, exercising the real synthesize command (not a copied routine).
 * Unscaled synthesis provides an exact fixed-point reference: mono output
 * must equal clamp16(2 * unscaled), with identical retained lap state.
 * No commercial audio, host encoder, filesystem, mixer, or AI is required.
 */
#include <libdragon.h>
#include <limits.h>
#include <stdint.h>
